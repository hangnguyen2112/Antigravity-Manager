...
class HomeView(ft.Container):
    ...
    def auto_backup(self):
        def task():
            # Trì hoãn một chút để đảm bảo UI đã tải xong
            time.sleep(1)
    ...
    def monitor_status(self):
        while self.running:
            ...
            if is_running:
                ...
                icon.color = "#34C759"
                text.value = "Antigravity đang chạy nền"
                text.color = "#34C759"
            else:
        ...
        def task():
            try:
                if switch_account(account_id):
                    ...
                    # self.show_message("Chuyển tài khoản thành công")
                else:
                    self.show_message("Chuyển tài khoản thất bại, vui lòng kiểm tra nhật ký", True)
            except Exception as e:
                import traceback
                error_msg = f"Lỗi khi chuyển tài khoản: {str(e)}\n{traceback.format_exc()}"
                from utils import error
                error(error_msg)
                self.show_message(f"Đã xảy ra lỗi: {str(e)}", True)
        threading.Thread(target=task, daemon=True).start()
    ...
    def delete_acc(self, account_id):
        def confirm_delete():
            try:
                if delete_account(account_id):
                    ...
                    self.show_message("Xóa tài khoản thất bại, vui lòng kiểm tra nhật ký", True)
            except Exception as e:
                import traceback
        ...
        self.show_confirm_dialog(
            title="Xác nhận xóa",
            content="Bạn có chắc muốn xóa bản sao lưu tài khoản này? Thao tác này không thể hoàn tác.",
